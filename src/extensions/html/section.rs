...
#[pyfunction]
pub fn chunk_html_section(py: Python<'_>, file_path: &str) -> PyResult<PyObject> {
    ...
    let chunks_raw = build_section_chunks(&bytes).map_err(PyRuntimeError::new_err)?;
...
pub fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(chunk_html_section, m)?)?; Ok(())
}
