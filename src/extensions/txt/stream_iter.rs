/// True streaming iterator for all TXT chunking strategies.
///
/// `structural` and `semantic` use block-by-block state machines:
///   - O(blocks) input, O(1) pending queue in the iterator struct.
///   - No pre-built chunk Vec.
///
/// `section`, `sliding_window`, `sentence`, `page_aware` use batch-drain:
///   - Batch function called once, result stored in iterator, drained one
///     chunk per __next__ call.  Identical output to the batch equivalent.
///
/// No threads required (pure Rust, no lifetime constraints).
...
impl StructuralState {
    fn new(blocks: Vec<TxtBlock>) -> Self {
        StructuralState {
            blocks, index: 0,
            current_heading: None,
            prose_parts: Vec::new(), prose_len: 0,
            pending_prose: None, done: false,
        }
    }
    ...
    fn flush_prose(&mut self) -> Vec<ChunkRecordInput> {
        ...
        let chunk = ChunkRecordInput { content_type: ct, content, metadata: txt_metadata(self.current_heading.clone()) };
    ...
    fn merge_prose(&mut self, chunk: ChunkRecordInput) -> Vec<ChunkRecordInput> {
        ...
        let is_prose = |ct: ContentType| matches!(ct,
        ...
        match block.content_type {
            ...
            ContentType::Table | ContentType::CodeBlock | ContentType::BulletNumberedList => {
                ...
                out.push(ChunkRecordInput {
                    ...
                    metadata: txt_metadata(self.current_heading.clone()),
                });
                Some(out)
            }
            _ => {
...
// Signal constants kept in sync with txt/semantic.rs.
...
const ELABORATION_STARTS: &[&str] = &[
    ...
    "on top of that", "notably", "importantly", "it is worth",
    "equally", "similarly", "likewise",
];
...
impl SemAccum {
    ...
    fn finalize(self, ci: usize, total: usize) -> ChunkRecordInput {
        ...
        ChunkRecordInput {
            ...
            content,
            metadata: json!({
                "section_heading": self.section_heading, "heading_path": self.heading_path,
                "section_level": self.section_level, "paragraph_count": self.parts.len(),
                "merge_reasons": merge_reasons, "primary_merge_reason": primary,
                "keyword_density": kd, "avg_block_length": avg_bl, "chunk_index": ci,
...
fn sem_decide(clean: &str, acc: &SemAccum) -> Option<&'static str> {
    ...
    if clean.len() <= 80 { return Some("short_paragraph"); }
    let bkw = tokenize_keywords(clean);
    if has_keyword_overlap(&acc.keywords, &bkw) { return Some("keyword_overlap"); }
    None
}

fn is_prose_ct(ct: ContentType) -> bool {
    matches!(ct, ContentType::PlainParagraph | ContentType::LongSingleParagraph | ContentType::ShortDisconnectedParagraph)
}
...
impl SemanticState {
    ...
    fn advance(&mut self) -> Option<Vec<ChunkRecordInput>> {
        ...
        if self.index >= self.blocks.len() {
            self.done = true;
            return Some(self.flush_accum().into_iter().collect());
        }
        let block = self.blocks[self.index].clone();
        self.index += 1;
        ...
        if block.content_type == ContentType::HeadingSection {
            ...
            out.push(ChunkRecordInput {
                ...
                metadata: json!({
                    "section_heading": current_section_heading(&self.heading_stack[..self.heading_stack.len()-1]),
                    "heading_path": heading_path_strings(&self.heading_stack),
                    "section_level": level, "paragraph_count": 0,
                    "merge_reasons": [], "primary_merge_reason": "initial",
                    "keyword_density": 0.0, "avg_block_length": text.len(),
                    "chunk_index": self.chunk_index,
                    "document_metadata": { "source_type": "txt", "total_input_blocks": self.total }
                }),
            });
            self.chunk_index += 1;
            Some(out)
        } else if !is_prose_ct(block.content_type) {
            let mut out: Vec<ChunkRecordInput> = self.flush_accum().into_iter().collect();
            out.push(ChunkRecordInput {
                content_type: block.content_type, content: block.content.clone(),
                metadata: json!({
            ...
            match self.accum.as_mut() {
                None => {
                    self.accum = Some(SemAccum::new(clean, current_section_heading(&self.heading_stack), heading_path_strings(&self.heading_stack), current_section_level(&self.heading_stack), bkws));
                    Some(Vec::new())
                }
...
#[pyclass]
pub struct TxtStreamIterator {
    backend: TxtStreamBackend,
    pending: VecDeque<ChunkRecordInput>,
    done: bool,
}

#[pymethods]
impl TxtStreamIterator {
    ...
    fn __next__(&mut self, py: Python<'_>) -> PyResult<Option<PyObject>> {
        loop {
            ...
            match &mut self.backend {
                TxtStreamBackend::Structural(s) => match s.advance() {
                    None => { self.done = true; }
                    Some(chunks) => { self.pending.extend(chunks); }
                },
                TxtStreamBackend::Semantic(s) => match s.advance() {
                    None => { self.done = true; }
                    Some(chunks) => { self.pending.extend(chunks); }
                },
                TxtStreamBackend::Batch(s) => match s.advance() {
                    None => { self.done = true; }
                    Some(chunk) => { return chunk_to_pydict(py, chunk).map(Some); }
                },
            }
        }
    }
...
pub fn stream_txt_chunks(
    ...
    match mode {
        ...
        _ => return Err(PyValueError::new_err(format!("Unknown TXT streaming mode: {mode}"))),
    }
    ...
    let backend = match mode {
        ...
        "section" => {
            let chunks = build_section_chunks(&bytes).map_err(PyRuntimeError::new_err)?;
            TxtStreamBackend::Batch(BatchDrainState::new(chunks))
        }
        "sliding_window" => {
    ...
    Py::new(py, TxtStreamIterator { backend, pending: VecDeque::new(), done: false })
}

pub fn register(m: &pyo3::Bound<'_, PyModule>) -> pyo3::PyResult<()> {
    m.add_class::<TxtStreamIterator>()?;
    m.add_function(wrap_pyfunction!(stream_txt_chunks, m)?)?;
    Ok(())
}
