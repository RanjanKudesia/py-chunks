...
pub fn extract_doc_images(file_path: &str) -> Result<Vec<DocImage>, String> {
    ...
    let data_stream = cfb_reader::read_data_stream(file_path).unwrap_or(None);
    let pieces =
...
#[pyfunction]
fn chunk_doc_structural_with_images(
    py: Python<'_>,
    file_path: &str,
) -> PyResult<(Vec<PyObject>, Vec<(String, Py<PyBytes>)>)> {
    chunk_doc_with_images_impl(py, file_path, build_structural_chunks)
}

#[pyfunction]
fn chunk_doc_section_with_images(
    py: Python<'_>,
    file_path: &str,
) -> PyResult<(Vec<PyObject>, Vec<(String, Py<PyBytes>)>)> {
    chunk_doc_with_images_impl(py, file_path, build_section_chunks)
}

#[pyfunction]
fn chunk_doc_semantic_with_images(
    py: Python<'_>,
    file_path: &str,
) -> PyResult<(Vec<PyObject>, Vec<(String, Py<PyBytes>)>)> {
    chunk_doc_with_images_impl(py, file_path, build_semantic_chunks)
}

#[pyfunction]
fn chunk_doc_sliding_window_with_images(
    py: Python<'_>,
    file_path: &str,
    window_size: usize,
    overlap: usize,
    ...
    chunk_doc_with_images_impl(py, file_path, move |p| {
        build_sliding_window_chunks(p, window_size, overlap)
    })
}

#[pyfunction]
fn chunk_doc_sentence_with_images(
    py: Python<'_>,
    file_path: &str,
...
#[pyfunction]
fn chunk_doc_page_aware_with_images(
    py: Python<'_>,
    file_path: &str,
...
pub(crate) fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(chunk_doc_structural_with_images, m)?)?;
    m.add_function(wrap_pyfunction!(chunk_doc_section_with_images, m)?)?;
    m.add_function(wrap_pyfunction!(chunk_doc_semantic_with_images, m)?)?;
    m.add_function(wrap_pyfunction!(chunk_doc_sliding_window_with_images, m)?)?;
    m.add_function(wrap_pyfunction!(chunk_doc_sentence_with_images, m)?)?;
    m.add_function(wrap_pyfunction!(chunk_doc_page_aware_with_images, m)?)?;
    Ok(())
}
...
mod tests {
    ...
    fn decode_picf_blip_reads_inline_png() {
        ...
        blip.extend_from_slice(&crate::extensions::odraw::RT_BLIP_PNG.to_le_bytes());
