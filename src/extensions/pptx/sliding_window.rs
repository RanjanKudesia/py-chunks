...
#[pyfunction]
pub fn chunk_pptx_sliding_window_with_images(
    py: Python<'_>,
    file_path: &str,
    window_size: usize,
    overlap: usize,
    ...
    let chunks_raw = build_sliding_window_chunks(&bytes, window_size, overlap)
        .map_err(|e| PyRuntimeError::new_err(format!("PPTX sliding-window chunking failed: {e}")))?;
    for c in chunks_raw {
        let dict = PyDict::new_bound(py);
        dict.set_item("content", &c.content)?;
        dict.set_item("content_type", c.content_type.as_str())?;
...
pub fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(chunk_pptx_sliding_window_with_images, m)?)?;
    Ok(())
}
