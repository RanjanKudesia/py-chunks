...
#[pyfunction]
pub fn chunk_md_section(py: Python<'_>, file_path: &str) -> PyResult<PyObject> {
    ...
    let chunks_raw = build_section_chunks(&bytes)
        .map_err(|e| PyRuntimeError::new_err(format!("MD section chunking failed: {e}")))?;
...
pub fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(chunk_md_section, m)?)?;
    Ok(())
}
