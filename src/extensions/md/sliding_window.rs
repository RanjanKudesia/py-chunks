...
#[pyfunction]
pub fn chunk_md_sliding_window(
    py: Python<'_>,
    file_path: &str,
    window_size: usize,
    overlap: usize,
    ...
    let chunks_raw = build_sliding_window_chunks(&bytes, window_size, overlap)
        .map_err(|e| PyRuntimeError::new_err(format!("MD sliding-window chunking failed: {e}")))?;
...
pub fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(chunk_md_sliding_window, m)?)?;
    Ok(())
}
