...
#[pyfunction]
pub fn chunk_md_semantic(py: Python<'_>, file_path: &str) -> PyResult<PyObject> {
    ...
    let chunks_raw = build_semantic_chunks(&bytes)
        .map_err(|e| PyRuntimeError::new_err(format!("Failed to parse MD semantically: {e}")))?;
...
pub fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(chunk_md_semantic, m)?)?;
    Ok(())
}
