...
pub struct CsvStreamIterator {
    receiver: mpsc::Receiver<Result<RawCsvChunk, String>>,
...
impl<R: Read> DecodingReader<R> {
    fn new(inner: R, encoding: &str) -> Result<Self, String> {
    ...
    fn decode_chunk(&self, bytes: &[u8]) -> Result<String, String> {
        match self.encoding {
...
fn chunk_to_py(py: Python<'_>, chunk: RawCsvChunk) -> PyResult<PyObject> {
...
fn decode_line_bytes(bytes: &[u8], encoding: &str) -> Result<String, String> {
...
fn detect_delimiter_from_file(file_path: &str, encoding: &str) -> Result<u8, String> {
...
fn open_csv_reader(
    ...
    delimiter: Option<u8>,
    encoding: &str,
) -> Result<(csv::Reader<Box<dyn Read + Send>>, u8), String> {
...
fn build_row_streaming(
    ...
    sender: mpsc::Sender<Result<RawCsvChunk, String>>,
) -> Result<(), String> {
    if rows_per_chunk == 0 {
...
fn run_worker(
    ...
    match mode.as_str() {
        ...
        _ => Err("mode must be 'row', 'default', 'sliding_window', or 'page_aware' for CSV streaming".to_string()),
    }
}

#[pymethods]
impl CsvStreamIterator {
    ...
    fn __next__(&mut self, py: Python<'_>) -> PyResult<Option<PyObject>> {
        ...
        match self.receiver.recv() {
            Ok(Ok(chunk)) => chunk_to_py(py, chunk).map(Some),
            Ok(Err(err)) => {
                self.done = true;
