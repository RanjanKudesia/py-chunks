...
use mail_parser::{Encoding, Address, Message, MessageParser, MimeHeaders, PartType};
...
pub fn parse_message_bytes(raw: &[u8]) -> EmlDocument {
    ...
    let Some(msg) = parsed else {
        // Unparseable — return an empty doc so callers degrade rather than error.
        return EmlDocument::default();
    };
    document_from_message(&msg)
}
